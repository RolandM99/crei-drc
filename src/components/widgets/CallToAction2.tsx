          ...
          <div className="flex items-center">
            {form.icon && (
              <span className="rounded-bl rounded-tl border-r-[1px] border-gray-400 px-2 py-2 dark:bg-[#3b3b3b]">
                <form.icon className="h-6 w-6 text-cyan-600 dark:text-yellow-800" />
              </span>
            )}
            <input
            ...
            <button
              type={form.btn.type}
              className="rounded-br rounded-tr border-l-[1px] border-gray-400 dark:bg-yellow-800 dark:text-slate-400 bg-cyan-600 px-4 py-2 text-white"
            >
              {form.btn.title}
            </button>
          </div>
...
const CallToAction2 = ({ title, subtitle, items }: CallToActionProps) => (
  <section className="bg-cyan-600 dark:bg-cyan-900 text-gray-200" id="callToActionTwo">
