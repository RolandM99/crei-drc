        ...
        <div className="flex items-stretch justify-center">
          <div className="grid grid-cols-3 gap-3 dark:text-white sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
            {testimonials.map(({ name, occupation, comment, image, icon: Icon, href }, index) => (
              ...
              >
                <div className="flex-flow card flex border-b-[3px] border-transparent text-center hover:border-cyan-600 hover:transition hover:duration-100">
                  <a
                    href={href}
                    target="_blank"
                  ...
                  >
                    ...
                    {href && Icon && <Icon className="mx-auto mt-4 h-6 w-6 text-cyan-600 dark:text-slate-200" />}
                  </a>
                </div>
              </div>
            ))}
